#!/usr/bin/env python3
"""
Coupled thermo‑mechanical FEA of piston crown (axisymmetric) – corrected.
Uses CAX4T element (coupled temperature‑displacement).
"""
import numpy as np
import subprocess
import os
...
heat_flux = 3.0                 # W/mm² (3 MW/m²) – more realistic peak
...
print("Generating axisymmetric thermo‑mechanical mesh (CAX4T)...")
...
inp_filename = "piston_crown_thermomech_v2.inp"
with open(inp_filename, "w") as f:
    # Header
    f.write("*HEADING\n")
    f.write("Coupled thermo‑mechanical analysis of piston crown (30 MPa + 3 MW/m²)\n")
    f.write("** Slothworks V12 at 3000 whp (corrected CAX4T)\n")
    # Nodes
    f.write("*NODE\n")
    for i, (r, z) in enumerate(nodes):
        f.write(f"{i+1}, {r:.6f}, {z:.6f}, 0.0\n")
    # Elements – coupled temperature‑displacement
    f.write("*ELEMENT, TYPE=CAX4T, ELSET=CROWN\n")
    ...
    f.write("1.0,\n")
    # Node set containing all nodes (for initial temperature)
    f.write("*NSET, NSET=ALLNODES\n")
    ...
    f.write("*FILM\n")
    f.write("BOTTOM, {}, {}\n".format(coolant_temp, convection_coeff))
    ...
    f.write("*TEMPERATURE, FILE=piston_crown_thermomech_v2, BSTEP=1\n")
...
try:
    ...
    print("ccx stdout (first 20 lines):")
    for line in result.stdout.split('\n')[:20]:
        if line.strip():
            print("   ", line)
    if result.stderr:
        print("ccx stderr (first 20 lines):")
    ...
    if result.returncode == 0:
        ...
        print(f"❌ CalculiX failed with exit code {result.returncode}.")
except subprocess.TimeoutExpired:
    print("❌ CalculiX timed out after 60 seconds.")
