...
Version 5: Enforce geometric feasibility bounds (pin boss width ≥32 mm, crown thickness ≥12 mm).
Relaxed mass constraint (1500 g), lattice density lower bound 0.6.
"""
import numpy as np
import random
import json
...
# Enforced geometric bounds (minimum pin boss width 32 mm, crown thickness 12 mm)
BOUNDS = {
    "crown_thickness": (12.0, 35.0),      # enforced minimum
    "pin_boss_width": (32.0, 60.0),       # enforced minimum (≥32 mm required)
    "skirt_length": (30.0, 100.0),        # unchanged
    "skirt_thickness": (2.0, 15.0),       # unchanged
...
def geometric_feasibility(geo):
    ...
    if geo.crown_thickness < 5.0:
        violations += 10
    # Pin boss width reasonable relative to pin diameter (already bounded)
...
def save_results(best, log, feasible, filename=None):
    ...
    if filename is None:
        ...
        filename = f"piston_opt_am_v5_results_{timestamp}.json"
...
def generate_cad(geometry, filename=None):
    ...
    if filename is None:
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"piston_optimized_am_v5_{timestamp}.step"
    
    try:
        from src.cad.piston_cad import create_piston, export_step
        from src.engine.piston import PistonGeometry
        # Convert PistonGeometryAM to PistonGeometry (same dimensions)
        geo_solid = PistonGeometry(
            bore_diameter=geometry.bore_diameter,
            compression_height=geometry.compression_height,
            pin_diameter=geometry.pin_diameter,
            pin_boss_width=geometry.pin_boss_width,
            crown_thickness=geometry.crown_thickness,
            ring_land_height=geometry.ring_land_height,
            ring_groove_depth=geometry.ring_groove_depth,
            skirt_length=geometry.skirt_length,
            skirt_thickness=geometry.skirt_thickness,
            # Material defaults to aluminum but we don't care for CAD
        )
        piston = create_piston(geo_solid)
...
if __name__ == "__main__":
    print("Starting piston generative design with ADDITIVE MANUFACTURING V5...")
    print(f"Material: forged steel with lattice infill (ρ_rel 0.6–1.0)")
    print(f"Enforced geometric bounds: pin boss width ≥32 mm, crown thickness ≥12 mm")
    ...
    # CAD generation if all constraints satisfied
    if all(cons.values()):
        print("\nAll constraints satisfied. Generating CAD...")
        # Generate CAD using the best geometry
        cad_filename = filename.replace('.json', '.step')
        generate_cad(geo, cad_filename)
    else:
