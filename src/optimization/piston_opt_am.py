...
Uses forged steel with lattice infill, relaxed constraints.
Variables: geometry + lattice relative density (0.2–1.0).
Constraints: mass <800 g, crown stress <0.8×yield, pin bearing pressure <100 MPa.
"""
import numpy as np
import random
import json
...
BOUNDS = {
    "crown_thickness": (8.0, 25.0),      # further expanded
    "pin_boss_width": (10.0, 30.0),      # further expanded
    "skirt_length": (30.0, 70.0),        # expanded
    "skirt_thickness": (2.0, 8.0),       # expanded
    "lattice_relative_density": (0.2, 1.0),
}
...
def evaluate_individual(vec):
    ...
    # Engineering analysis with AM‑aware constraints
...
def save_results(best, log, feasible, filename=None):
    ...
    if filename is None:
        ...
        filename = f"piston_opt_am_results_{timestamp}.json"
...
if __name__ == "__main__":
    print("Starting piston generative design with ADDITIVE MANUFACTURING...")
    print(f"Material: forged steel with lattice infill (ρ_rel 0.2–1.0)")
    print(f"Relaxed thresholds: crown stress <0.8×yield, pin bearing pressure <100 MPa, mass <800 g")
