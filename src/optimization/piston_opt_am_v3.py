...
Version 3: Expanded geometry bounds, start from best seed (seed 2).
Material: forged steel with lattice infill, constraints unchanged.
"""
import numpy as np
import random
import json
...
# Expanded design variable bounds (mm) + lattice density lower bound 0.5
BOUNDS = {
    ...
    "lattice_relative_density": (0.5, 1.0),  # unchanged
}
...
def save_results(best, log, feasible, filename=None):
    ...
    if filename is None:
        ...
        filename = f"piston_opt_am_v3_results_{timestamp}.json"
...
if __name__ == "__main__":
    print("Starting piston generative design with ADDITIVE MANUFACTURING V3...")
    print(f"Material: forged steel with lattice infill (ρ_rel 0.5–1.0)")
    print(f"Constraints: crown stress <0.8×yield, pin bearing pressure <100 MPa, mass <800 g")
