"""
Generative design optimization for V12 connecting rod with relaxed constraints.
Relaxed thresholds: mass <1.5 kg, bearing pressure <120 MPa, buckling SF ≥1.5,
fatigue SF ≥1.5, stress limit 0.6×yield (528 MPa).
Uses RelaxedConrodAnalyzer.
"""
import numpy as np
import random
import json
...
from src.engine.conrod_relaxed import RelaxedConrodAnalyzer
...
def evaluate_individual(vec):
    ...
    # 3. Engineering analysis with RELAXED constraints
    analyzer = RelaxedConrodAnalyzer(geo)
...
def save_results(best, log, feasible, filename=None):
    ...
    if filename is None:
        ...
        filename = f"conrod_opt_relaxed_results_{timestamp}.json"
...
if __name__ == "__main__":
    print("Starting connecting‑rod generative design with RELAXED constraints...")
    print(f"Relaxed thresholds: mass <1.5 kg, bearing pressure <120 MPa, buckling SF ≥1.5, fatigue SF ≥1.5, stress limit 0.6×yield")
