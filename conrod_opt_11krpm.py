...
Updated 11k RPM loads (compression 180 kN, tensile 83 kN), expanded bounds.
...
# Expanded design variable bounds (mm) – already expanded
BOUNDS = {
...
def save_results(best, log, feasible, filename=None):
    ...
    if filename is None:
        ...
        filename = f"conrod_opt_results_11krpm_{timestamp}.json"
...
def generate_cad(geometry, filename="conrod_optimized_11krpm.step"):
...
if __name__ == "__main__":
    print("Starting connecting‑rod generative design optimization with 11k RPM loads...")
    ...
    if all(cons.values()):
        ...
        cad_filename = f"conrod_optimized_11krpm_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.step"
