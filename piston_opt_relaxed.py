"""
Generative design optimization for V12 piston with RELAXED constraints.
Relaxed constraints: crown stress <0.8×yield (248 MPa), pin bearing pressure <80 MPa.
Expanded bounds: crown thickness 8-20 mm, pin boss width 10-25 mm.
Use DEAP with 20 population, 15 generations.
...
# Design variable bounds (mm) – expanded as requested
BOUNDS = {
    "crown_thickness": (8.0, 20.0),      # expanded from (6,15)
    "pin_boss_width": (10.0, 25.0),      # expanded from (8,20)
    "skirt_length": (30.0, 60.0),        # unchanged
    "skirt_thickness": (2.0, 6.0),       # unchanged
}
...
class RelaxedPistonAnalyzer(PistonAnalyzer):
    """Piston analyzer with relaxed constraints."""
    def evaluate_constraints(self, peak_pressure_mpa: float,
                            peak_force_n: float,
                            tensile_force_n: float = None) -> tuple:
        """Evaluate design constraints with relaxed limits."""
        ...
        # RELAXED constraints
        crown_yield = self.geo.yield_strength * 0.8   # 0.8×yield (248 MPa)
        pin_bearing_limit = 80.0  # MPa (relaxed from 60)
        constraints = {}
        constraints["crown_stress_ok"] = metrics["crown_bending_mpa"] < crown_yield
        constraints["pin_bearing_comp_ok"] = metrics["pin_bearing_comp_mpa"] < pin_bearing_limit
        constraints["pin_bearing_tens_ok"] = metrics["pin_bearing_tens_mpa"] < pin_bearing_limit
        constraints["mass_ok"] = metrics["mass_g"] < 500.0  # target <500g (unchanged)
...
def evaluate_individual(vec):
    ...
    analyzer = RelaxedPistonAnalyzer(geo)
...
def save_results(best, log, feasible, filename=None):
    ...
    if filename is None:
        ...
        filename = f"piston_opt_results_relaxed_{timestamp}.json"
...
def generate_cad(geometry, filename=None):
    ...
    if filename is None:
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"piston_optimized_relaxed_{timestamp}.step"
...
if __name__ == "__main__":
    print("Starting piston generative design optimization with RELAXED constraints...")
    ...
    print("Relaxed constraints: crown stress <0.8×yield (248 MPa), pin bearing pressure <80 MPa")
